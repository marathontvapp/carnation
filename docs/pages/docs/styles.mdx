...
<Callout type="info">
  ...
  for the big two of your design system styles: color and typography. In the
  following examples, we will define our tokens in the root `theme` config to
  override the default config values. If you'd rather keep the default Tailwind
  config as well, drop the following into `theme.extend`.
</Callout>
...
<Callout type="warning">
  Avoid using named colors like `transparent` and `currentColor` in values as
  these won't transfer nicely to native. Rather, use [8-digit hex
...
<c.button className="text-dark bg-light">
  <c.svg className="fill-emerald-400">...</c.svg>
</c.button>
```
...
```tsx filename="packages/ui/Article.tsx"
<c.article>
...
```tsx filename="packages/ui/BugIcon.tsx"
...
```tsx filename="packages/ui/ReportBugButton.tsx"
...
import type { SpringTransition, BezierDefinition } from "carnation/motion";
...
const linear: BezierDefinition = [0, 0, 1, 1];
...
export const EASE = Object.freeze({ linear, easIn, easeOut, easeInOut });
...
```tsx filename="packages/ui/Example.tsx"
